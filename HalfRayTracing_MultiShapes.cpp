...
int main()
{
  ...
  shapes[4] = new Sphere(Vec3(-1, 0, 5), 1, defaultMt);
  shapes[5] = new Plane(Vec3(0,5,0),Vec3(0,-1,0),floorPlaneMt);
  Color envLight(255,255,255);
  PointLightSource* lights[3];
  lights[0] = new PointLightSource(Vec3(-5, -10, -5), Color(0,255,0));
  lights[1] = new PointLightSource(Vec3(5, 0, -5), Color(255,0,0));
  lights[2] = new PointLightSource(Vec3(0, 5, -5), Color(0,0,255));
  int W = 512;
  int H = 512;
  PPM image(W, H, 255);
  Material mt();
  for (double y = 0; y < H; y += 1)
  {
    ...
    {
      Ray camera(0, 0, 0, x / W - 0.5, y / H - 0.5, 1);
  ...
  image.writePPM("HalfRayTrace.ppm");
}